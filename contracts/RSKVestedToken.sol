...
contract RSkVestedToken is StandardToken, LimitedTransferToken {
    ...
    } // total 86 bytes = 3 sstore per operation (32 per sstore)
     ...
     * @param _lockPeriod uint64 The lockup period
   ...
   */
    ...
    function transferableTokens(address holder, uint64 time) public constant returns (uint256) {
        ...
        for (uint256 i = 0; i < grantIndex; i++) {
            TokenGrant storage grant = grants[holder][i];
            if (time >= grant.lockPeriod) {
                nonVested = SafeMath.add(nonVested, nonVestedTokens(grant, time));
            } else {
                nonVested = grant.value;
            }
//            nonVested = SafeMath.add(nonVested, nonVestedTokens(grants[holder][i], time));
     ...
     * @param _holder The address which will have its tokens revoked.
     * @param _grantId The id of the token grant.
     * @return Returns all the values that represent a TokenGrant(address, value, lockPeriod, start, cliff,
     * revokability, burnsOnRevoke, and vesting) plus the vested value at the current time.
     */
    function tokenGrant(address _holder, uint256 _grantId) public constant returns (address granter, uint256 value, uint256 vested, uint64 lockPeriod, uint64 start, uint64 cliff, uint64 vesting, bool revokable, bool burnsOnRevoke) {
        TokenGrant storage grant = grants[_holder][_grantId];

        granter = grant.granter;
        value = grant.value;
        lockPeriod = grant.lockPeriod;
        start = grant.start;
        cliff = grant.cliff;
    ...
    function lastTokenIsTransferableDate(address holder) public constant returns (uint64 date) {
        ...
        for (uint256 i = 0; i < grantIndex; i++) {
            ...
            date = Math.max64(grants[holder][i].lockPeriod, date);
        }
    }
}
